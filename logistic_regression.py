...
class lr_model():
    ...
    d_best = None
    device = device = torch.accelerator.current_accelerator() if \
    ...
    def train(self, best=True, model_file=None, num_epoch=None, save_model=False):
        ...
        for i_epoch in range(self.n_epoch):
            ...
            # backwards
                ...
                if (i_step+1) % 100 == 0:
                    self.print (f"  epoch [{i_epoch+1}/{self.n_epoch}], "+
                                f"step [{i_step+1}/{n_step}], loss: {loss.item():.4f}")
...
#+
#-
def test_logistic_regression(save_model=False):
    print("---------- testing model ----------")
    dir = "C:\\data"
    nn_model = lr_model(root_folder=dir, verbose=True)
    nn_model.train()
